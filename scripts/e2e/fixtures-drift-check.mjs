 ...
 *   - tests/fixtures/raw/id-map.json generated by `npm run fixtures:generate`
 *
 * Example:
...
if (!fs.existsSync(idMapPath)) {
  console.error('Error: Missing tests/fixtures/raw/id-map.json.');
  console.error('Run fixtures capture + generate first to create stable→live metadata mapping:');
  console.error('  1) npm run fixtures:capture');
  console.error('  2) npm run fixtures:generate');
  process.exit(1);
}

const routes = JSON.parse(fs.readFileSync(routesPath, 'utf8'));
const idMap = JSON.parse(fs.readFileSync(idMapPath, 'utf8'));
const stablePathToOriginalPath = idMap?.stablePathToOriginalPath || {};

// Filter to routes with fixtures (not error simulations), and resolve live metadata keys
const rawCheckRoutes = Object.entries(routes)
  .filter(([routePath, config]) => config.fixture && !config.errorStatus)
  .map(([routePath, config]) => ({
    routePath,
    livePath: stablePathToOriginalPath[routePath] || null,
    fixture: config.fixture,
    title: config.title
  }));

const checkRoutes = [];
for (const route of rawCheckRoutes) {
  if (!route.livePath) {
    console.log(`  ⚠ Skipping ${route.routePath}: no live key mapping in raw/id-map.json`);
    continue;
  }
...
console.log(`Resolved ${checkRoutes.length}/${rawCheckRoutes.length} routes with live key mappings.\n`);
...
for (const route of checkRoutes) {
  ...
  // Fetch live response using original metadata key mapping
  try {
    const url = `${PLEX_SERVER}${route.livePath}?includeGuids=1&includeExternalMedia=1&X-Plex-Token=${PLEX_TOKEN}`;
    const response = await fetch(url);

    if (!response.ok) {
      console.log(`  ✗ ${route.routePath} (live ${route.livePath}): HTTP ${response.status}`);
      continue;
    }
    ...
    if (added.length > 0 || removed.length > 0) {
      ...
      console.log(`  ⚠ DRIFT: ${route.routePath} (live ${route.livePath}, ${route.title})`);
      if (added.length > 0) console.log(`      Added: ${added.join(', ')}`);
      if (removed.length > 0) console.log(`      Removed: ${removed.join(', ')}`);
    } else {
      console.log(`  ✓ ${route.routePath} (live ${route.livePath}, ${route.title})`);
    }
  } catch (error) {
    console.log(`  ✗ ${route.routePath}: ${error.message}`);
  }
}
...
if (driftFound) {
  ...
  for (const item of driftReport) {
    console.log(`📌 ${item.path} (live ${item.livePath}, ${item.title})`);
